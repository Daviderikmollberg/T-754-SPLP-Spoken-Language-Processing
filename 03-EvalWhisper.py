...
def main(args):
    ...
    for idx, out in enumerate(whisper_asr(data(dataset), batch_size=batch_size)):
        ...
        print(f"{idx}:\t{prediction}\t{reference}")

    res = eval_metric.compute(references=references, predictions=predictions)
    res = round(100 * res, 2)
...
if __name__ == "__main__":
    ...
    print(
        "Arguments:\n",
        "\n".join(f"{str(k).strip()}\t{str(v).strip()}" for k, v in vars(args).items()),
        "\n",
    )
